...
function usces_pdf_out($pdf, $data){
	...
	for ( $index = 0; $index < count($cart); $index++ ) {
		...
		$fontsize = apply_filters( 'useces_filter_order_pdfbody_fontsize', $fontsizes, $args, $data );

		$line_y[$index] = $next_y;

		list($fontsize, $lineheight, $linetop) = usces_set_font_size($fontsizes['item_name']);//10->8
